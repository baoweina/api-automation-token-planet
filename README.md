...
api/
  ...
  startup_api.py           项目认筹记录接口封装
testcases/
  allure_labels.py        Epic/Feature 常量（Epic：接口自动化测试；
                          Feature：用户登录接口 / 首页接口 / 项目认筹记录接口）
  ...
  test_vote_record.py      Feature: 项目认筹记录接口（投资方认筹记录分页）
utils/
...
- `testcases/test_vote_record.py` — Feature: **项目认筹记录接口**
  （`POST /exio/startup/vote-record/list`）
  - 反向场景（不依赖登录态，可无人值守运行）：缺少必填字段 `startupId`、
    `pageSize` 越界(<1 或 >100) 均返回 `1100` 参数校验错误（**参数校验优先于鉴权**，
    跟首页接口的校验顺序不同，已实测确认）；参数合法但未登录返回 `2113`；
    携带无效/伪造 Token 返回 `2110`（跟首页接口对无效 Token 的返回码也不同，
    不要跨接口假设一致的错误码）
  - 正向场景（`needs_auth`，依赖 `AUTH_TOKEN`）：成功查询并做响应结构校验、
    `pageSize` 边界最大值(100) 正常返回、`seasonId=0` 查询所有赛季、
    查询几乎不存在的 `startupId` 时仍正常返回空列表
...
- 不同接口的"参数校验 vs 鉴权检查"先后顺序、以及无效 Token 的错误码，**不能跨接口假设一致**：
  `/exio/demoday/live` 是鉴权检查优先（未登录时哪怕参数非法也先返回 `2113`）；
  而 `/exio/startup/vote-record/list` 是参数校验优先（必填缺失/越界即使未登录也先返回
  `1100`，只有参数合法时才会走到鉴权检查返回 `2113`），且无效 Token 返回的是 `2110`
  而不是 `2113`。写新接口用例前建议先用 curl 实测确认真实返回码，再据此写断言
